...
- gsap.to()：这个方法用于定义一个元素从当前状态到指定结束状态的动画。它是最常用的动画方法，因为它通常适用于你希望元素以某种方式移动或变化到一个新的状态。
- gsap.from()：与to相反，from方法定义了一个元素从一个指定的起始状态过渡到它的当前状态。这对于当你想要某个元素立即呈现某种效果然后回到正常状态时非常有用。
- gsap.fromTo()：这个方法允许你同时指定元素的起始状态和结束状态。这在你需要明确控制元素在整个动画过程中的起点和终点时非常有用。